...
namespace Engine
{
	...
	physx::PxFilterFlags EngineFilterShader(physx::PxFilterObjectAttributes attributes0, physx::PxFilterData filterData0, physx::PxFilterObjectAttributes attributes1, physx::PxFilterData filterData1, physx::PxPairFlags& pairFlags, const void* constantBlock, physx::PxU32 constantBlockSize)
	{
	...
	void PhysicsMeshSerializer::SerializeMesh(const std::string& filepath, const physx::PxDefaultMemoryOutputStream& data, const std::string& submeshName)
	{
		...
		ENGINE_INFO("Serializing {0}", submeshName);	

		FILE* f = fopen(cachedFilepath.c_str(), "wb");
		if (f)
		{
			ENGINE_INFO("File Created");
			fwrite(data.getData(), sizeof(physx::PxU8), data.getSize() / sizeof(physx::PxU8), f);
			fclose(f);
		}
		else
		{
			ENGINE_INFO("File Already Exists");
		}
	}
