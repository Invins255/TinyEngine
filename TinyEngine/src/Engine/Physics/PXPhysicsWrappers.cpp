...
namespace Engine
{
	...
	void PhysicsErrorCallback::reportError(physx::PxErrorCode::Enum code, const char* message, const char* file, int line)
	{
		...
		case physx::PxErrorCode::eDEBUG_INFO:
			ENGINE_INFO("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
			break;
		case physx::PxErrorCode::eDEBUG_WARNING:
		case physx::PxErrorCode::ePERF_WARNING:
			ENGINE_WARN("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
			break;
		case physx::PxErrorCode::eINVALID_PARAMETER:
		case physx::PxErrorCode::eINVALID_OPERATION:
		case physx::PxErrorCode::eOUT_OF_MEMORY:
		case physx::PxErrorCode::eINTERNAL_ERROR:
			ENGINE_ERROR("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
			break;
		case physx::PxErrorCode::eABORT:
		case physx::PxErrorCode::eMASK_ALL:
			ENGINE_FATAL("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
			ENGINE_ASSERT(false, "");
			break;
		}
	}
	...
	{
		ENGINE_ERROR("[PhysX Error]: {0}:{1} - {2}", file, line, exp);
	}
	...
	void ContactListener::onWake(physx::PxActor** actors, physx::PxU32 count)
	{
		for (uint32_t i = 0; i < count; i++)
		{
			...
			ENGINE_INFO("PhysX Actor waking up: ID: {0}, Name: {1}", entity.ID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	void ContactListener::onSleep(physx::PxActor** actors, physx::PxU32 count)
	{
		for (uint32_t i = 0; i < count; i++)
		{
			...
			ENGINE_INFO("PhysX Actor going to sleep: ID: {0}, Name: {1}", entity.ID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	physx::PxScene* PXPhysicsWrappers::CreateScene()
	{
		...
		ENGINE_ASSERT(sceneDesc.isValid(), "");
	...
	void PXPhysicsWrappers::AddBoxCollider(PhysicsActor& actor)
	{
		...
		glm::vec3 size = actor.m_Entity.GetTransformComponent().Scale;
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateConvexMesh(MeshColliderComponent& collider, const glm::vec3& size, bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collider.CollisionMesh->GetFilePath()))
		{
			...
			for (const auto& submesh : collider.CollisionMesh->GetSubmeshes())
			{
				...
				{
					ENGINE_ERROR("Failed to cook convex mesh {0}", submesh.MeshName);
					continue;
				}
		...
		if (collider.ProcessedMeshes.size() <= 0)
		{
			for (auto shape : shapes)
			{
				...
				for (uint32_t i = 0; i < nbPolygons; i++)
				{
					...
					collider.ProcessedMeshes.push_back(CreateRef<Mesh>(collisionVertices, collisionIndices, FromPhysXTransform(shape->getLocalPose())));
				}
			}
		}
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateTriangleMesh(MeshColliderComponent& collider, const glm::vec3& size, bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collider.CollisionMesh->GetFilePath()))
		{
			...
			for (const auto& submesh : collider.CollisionMesh->GetSubmeshes())
			{
				...
				{
					ENGINE_ERROR("Failed to cook triangle mesh: {0}", submesh.MeshName);
					continue;
				}
		...
		if (collider.ProcessedMeshes.size() <= 0)
		{
			for (auto shape : shapes)
			{
				...
				collider.ProcessedMeshes.push_back(CreateRef<Mesh>(vertices, indices, FromPhysXTransform(shape->getLocalPose())));
			}
		}
	...
	{
		ENGINE_ASSERT(!s_Foundation, "PXPhysicsWrappers::Initializer shouldn't be called more than once!");

		s_Foundation = PxCreateFoundation(PX_PHYSICS_VERSION, s_Allocator, s_ErrorCallback);
		ENGINE_ASSERT(s_Foundation, "PxCreateFoundation Failed!");
		...
		ENGINE_ASSERT(s_Physics, "PxCreatePhysics Failed!");

		s_CookingFactory = PxCreateCooking(PX_PHYSICS_VERSION, *s_Foundation, s_Physics->getTolerancesScale());
		ENGINE_ASSERT(s_CookingFactory, "PxCreatePhysics Failed!");
