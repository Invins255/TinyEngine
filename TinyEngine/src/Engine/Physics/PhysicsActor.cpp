...
namespace Engine
{
	...
	float PhysicsActor::GetMass() const
	{
		if (!IsDynamic())
		{
			ENGINE_WARN("Trying to set mass of non-dynamic PhysicsActor.");
			return 0.0f;
		}
	...
	void PhysicsActor::AddForce(const glm::vec3& force, ForceMode forceMode)
	{
		if (!IsDynamic())
		{
			ENGINE_WARN("Trying to add force to non-dynamic PhysicsActor.");
			return;
		}
	...
	void PhysicsActor::AddTorque(const glm::vec3& torque, ForceMode forceMode)
	{
		if (!IsDynamic())
		{
			ENGINE_WARN("Trying to add torque to non-dynamic PhysicsActor.");
			return;
		}
	...
	glm::vec3 PhysicsActor::GetLinearVelocity() const
	{
		if (!IsDynamic())
		{
			ENGINE_WARN("Trying to get velocity of non-dynamic PhysicsActor.");
	...
	void PhysicsActor::SetLinearVelocity(const glm::vec3& velocity)
	{
		if (!IsDynamic())
		{
			ENGINE_WARN("Trying to set velocity of non-dynamic PhysicsActor.");
			return;
		}
	...
	glm::vec3 PhysicsActor::GetAngularVelocity() const
	{
		if (!IsDynamic())
		{
			ENGINE_WARN("Trying to get angular velocity of non-dynamic PhysicsActor.");
			return glm::vec3(0.0f);
		}
	...
	void PhysicsActor::SetAngularVelocity(const glm::vec3& velocity)
	{
		if (!IsDynamic())
		{
			ENGINE_WARN("Trying to set angular velocity of non-dynamic PhysicsActor.");
			return;
		}
	...
	void PhysicsActor::SynchronizeTransform()
	{
		if (IsDynamic())
		{
			TransformComponent& transform = m_Entity.GetTransformComponent();
